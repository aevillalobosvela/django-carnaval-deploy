...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const modalMar = document.getElementById('modalMar');
  const modalM = new bootstrap.Modal(modalMar);
  const btnFlotanteMar = document.getElementById('modalMarBtn');
  const ocultarModalMar = localStorage.getItem('ocultarModalMar');

  if (!ocultarModalMar) {
    modalM.show();
  } else {
    btnFlotanteMar.style.display = 'block';
  }

  // Guardar preferencia si se marcó el checkbox
  document.getElementById('cerrarmodalMarBtn').addEventListener('click', function () {
    const checkMar = document.getElementById('noMostrarCheckMar');
    if (checkMar.checked) {
      localStorage.setItem('ocultarModalMar', 'true');
    }
  });

  // Reabrir modal con el botón flotante
  btnFlotanteMar.addEventListener('click', () => {
    modalM.show();
  });
